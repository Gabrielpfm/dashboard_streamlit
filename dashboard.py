...
#comandos pra iniciar o app
#1 - .\venv\Scripts/activate (inicia o ambiente virtual)
#2 - streamlit run dashboard.py (inciao o programa)
# Ctrl + C fecha o streamlit
...
#Feito antes de puxar os dados pois a api permite colocar ese valores no url pra filtro
querry_string = {'regiao': regiao.lower(),'ano': ano}
response = requests.get(url,params= querry_string)
dados = pd.DataFrame.from_dict(response.json())
dados['Data da Compra'] = pd.to_datetime(dados['Data da Compra'],format='%d/%m/%Y')

#feito depois de puxar os dados porque não é possível filtrar diretamente da api
...
receita_mensal = dados.set_index("Data da Compra")\
                     .groupby(pd.Grouper(freq = 'ME'))['Preço'].sum().reset_index() #agrupa indices que possuem data por meio dos meses
...
#Gráfico da receita por estado
fig_receita_estados = px.bar(receita_estados.head(5),
                             x= 'Local da compra',
                             y= 'Preço',
                             text_auto  = True,
                             title = 'Top 5 estados (receitas)'
                             )
...
with aba1:
    ...
    with col2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0])) #qtd de linhas
...
#Infos na aba 3
with aba3:
    qtd_vendedores = st.number_input("Quantidade de vendedores",2,10,5) #min,max,inicial
    col1,col2 = st.columns(2)
    with col1: 
        st.metric('Receita',formata_numero(dados['Preço'].sum(),'R$'))
...
#st.map(dados) cria o mapa com pontos nas latitudes e longitudes apresentadas no datafram mas com bolinhas unicas e com mesmo tamanho



#st.dataframe(dados) # se so escrever dados, pro conta do "Magic" do streamlit ja identifica automatico como dataframe e aparece
